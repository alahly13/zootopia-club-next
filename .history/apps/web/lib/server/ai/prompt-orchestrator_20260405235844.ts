...
export function buildAssessmentPrompt(input: {
  ...
  const supportedTypeList =
    "mcq | true_false | essay | fill_blanks | short_answer | matching | multiple_response";
  ...
  const lines = [
    ...
    `Provider runtime configured: ${input.providerConfigured ? "yes" : "no"}`,
     ...
     "Structure instructions: format each question according to its type (MCQ options, explicit True/False statements, essay prompts, blanks, matching pairs, or multiple-response choices) and keep answer/rationale aligned with that type.",
