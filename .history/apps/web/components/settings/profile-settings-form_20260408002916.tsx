...
function buildPhoneCountryOptions(options: ProfileCountryOption[]) {
...
function extractNationalDigitsFromPhoneValue(
  ...
  const normalizedValue = normalizePhoneForCompare(value);
...
function formatPhonePreview(value: string) {
  ...
  try {
    return formatPhoneNumberIntl(normalizedValue);
  } catch {
...
export function ProfileSettingsForm({
  ...
  const [phoneValue, setPhoneValue] = useState(
    normalizePhoneForCompare(initialPhoneNumber),
  );
  ...
  const originCountryOptions = useMemo(
    () => buildCountryFieldOptions(profileCountryOptions, originCountry),
    [originCountry, profileCountryOptions],
  );

  const normalizedPhoneValue = useMemo(
    () => normalizePhoneForCompare(phoneValue),
    [phoneValue],
  );

  const phonePreview = useMemo(
    () => formatPhonePreview(normalizedPhoneValue),
  ...
  const isPhoneVerified =
    ...
    normalizedPhoneValue.length > 0 &&
    verifiedPhoneNumber === normalizedPhoneValue;
  ...
  const requirementItems = useMemo(
    () =>
      [
        ...
        {
          key: "originCountry",
          label: messages.settingsOriginCountryLabel,
          icon: MapPinned,
          complete: Boolean(normalizeTextValue(originCountry)),
        },
      ] satisfies {
  ...
  const missingRequirementCount = requirementItems.filter(
    (item) => !item.complete,
  ).length;
  ...
  function updatePhoneDraft(nextCountryIso2: string, nextPhoneValue: string) {
    const resolvedCountry = resolveProfileCountryOption(
      profileCountryOptions,
      nextCountryIso2,
    );
    const normalizedNextPhoneValue = normalizePhoneForCompare(nextPhoneValue);

    setSelectedPhoneCountryIso2(resolvedCountry.iso2);
    setPhoneValue(normalizedNextPhoneValue);
    ...
    /* Phone verification is tied to one exact normalized E.164 number.
       If the student edits the country or digits after a successful OTP, clear the verified snapshot here so the server-enforced completion gate can't be bypassed with a stale verification state. */
    if (normalizedNextPhoneValue !== verifiedPhoneNumber) {
  ...
  function handlePhoneCountryChange(nextCountryIso2: string) {
    const currentCountry = selectedPhoneCountry;
    const nextCountry = resolveProfileCountryOption(
      profileCountryOptions,
      nextCountryIso2,
    );
    const carriedNationalDigits = extractNationalDigitsFromPhoneValue(
      phoneValue,
      currentCountry,
    );
    const nextPhoneValue = carriedNationalDigits
      ? `+${nextCountry.callingCode}${carriedNationalDigits}`
      : "";

    updatePhoneDraft(nextCountry.iso2, nextPhoneValue);
  }
  ...
  async function handleSendVerificationCode() {
    ...
    const phoneValidation = validatePhoneNumberE164(normalizedPhoneValue);
  ...
  async function handleConfirmVerificationCode() {
    ...
    try {
      ...
      const persistedPhone = payload.data.user.phoneNumber ?? normalizedPhoneValue;
      ...
      setPhoneValue(normalizePhoneForCompare(persistedPhone));
  ...
  return (
    <section className="relative overflow-hidden rounded-[2.5rem] border border-white/25 bg-white/65 p-6 shadow-[0_30px_90px_rgba(2,6,23,0.08)] backdrop-blur-2xl dark:border-white/8 dark:bg-slate-950/45 md:p-8">
      ...
      <div className="relative z-10">
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit} noValidate>
          ...
          <section className={panelClassName}>
            ...
            <div className="mt-5 grid gap-4 xl:grid-cols-[minmax(0,18rem)_minmax(0,1fr)]">
              <SettingsCountrySelect
              ...
              <div className="space-y-3">
                <div className="flex min-h-[4.1rem] items-center gap-3 rounded-[1.25rem] border border-slate-200/80 bg-white/90 px-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.48),0_14px_28px_rgba(2,6,23,0.05)] transition focus-within:border-emerald-500/40 focus-within:bg-white dark:border-slate-700/70 dark:bg-slate-950/75 dark:focus-within:border-emerald-400/40">
                  ...
                  <PhoneNumberInput
                    country={selectedPhoneCountry.iso2 as Country}
                    international={false}
                    value={normalizedPhoneValue || undefined}
                    onChange={(nextValue) => {
                      updatePhoneDraft(
                        selectedPhoneCountry.iso2,
                        normalizePhoneForCompare(nextValue ?? ""),
                      );
                    }}
            ...
            <div className="mt-4 rounded-[1.4rem] border border-slate-200/80 bg-white/72 p-4 shadow-[0_16px_38px_rgba(2,6,23,0.06)] dark:border-slate-800 dark:bg-slate-950/68">
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            ...
            <div className={panelClassName}>
              <SettingsCountrySelect
                label={messages.settingsOriginCountryLabel}
                value={originCountry}
                placeholder={messages.settingsOriginCountryPlaceholder}
                searchPlaceholder={messages.settingsCountrySearchPlaceholder}
                searchEmpty={messages.settingsCountrySearchEmpty}
                options={originCountryOptions}
                icon={MapPinned}
                onChange={(nextValue) => {
                  setOriginCountry(nextValue);
