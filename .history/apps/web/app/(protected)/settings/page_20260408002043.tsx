...
export default async function SettingsPage({ searchParams }: SettingsPageProps) {
  ...
  const nationalityOption = user.nationality
    ...
    : null;
  const originCountryOption = user.originCountry
    ? findProfileCountryOptionByCanonicalLabel(countryOptions, user.originCountry)
    : null;
            ...
            <div className="mt-4 space-y-2.5">
              ...
              {originCountryOption ? (
                <div className="flex items-center gap-2.5 rounded-[1rem] border border-white/40 bg-white/80 px-3 py-2.5 dark:border-white/10 dark:bg-slate-950/70">
                  ...
                  <span className="text-sm text-foreground-muted">
                    {originCountryOption.flag} {originCountryOption.label}
                  </span>
                </div>
              ) : null}
            </div>
